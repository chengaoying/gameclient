...
public class ByteBuffer {
	...
	public int asynSlurp(InputStream is) throws IOException {
		int count = 0;
		int n = 0;
		while (availabe() > 0) {
			...
			if (isLen > 0) {
				...
				if ((n = is.read(buffer, writerIndex, expLen)) > 0) {
					writerIndex += n;
					count += n;
				}
			} else {
	...
	public int slurp(InputStream is) throws IOException {
		...
		int n = 0;
		while (availabe() > 0
				&& -1 != (n = is.read(buffer, writerIndex, availabe()))) {
			writerIndex += n;
			count += n;
		}
		if (n == -1 && count == 0) {
	...
	public int getMarkReaderIndex() {
		return markReaderIndex;
	}

	public int getMarkWriterIndex() {
	...
	public void resumeReaderIndex() {
		this.readerIndex = markReaderIndex;
	}
	...
	public void resumeWriterIndex() {
		this.writerIndex = markWriterIndex;
	}
	...
	public boolean dataEquals(ByteBuffer other) {
		...
		if (length() == other.length()) {
			for (int i = readerIndex; i < writerIndex; ++i) {
				if (buffer[readerIndex + i] != other.buffer[other.readerIndex
						+ i]) {
					return false;
				}
			}
